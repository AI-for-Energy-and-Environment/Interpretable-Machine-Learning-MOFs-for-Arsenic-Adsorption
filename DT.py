...
def bo_params_dt(max_depth, min_samples_leaf, min_samples_split):
    ...
    reg = tree.DecisionTreeRegressor(random_state=random_state, **params)
    ...
    save_path = '.../...'
    file_path = os.path.join(save_path, 'DT1optimization_output.md')
...
def run_bayesian_optimization(X_train, y_train):
    ...
    dt_BO.maximize(n_iter=200, init_points=20, acquisition_function=uf)
...
def cross_validate_model(X, Y, params):
    dt_run = tree.DecisionTreeRegressor(random_state=0, **params)
    ...
    for i, (tr_idx, te_idx) in enumerate(kf.split(X, Y)):
        ...
        dt_run.set_params(random_state=random_state)
        dt_run.fit(X_train, y_train)
        y_tr_pred = dt_run.predict(X_train)
        y_te_pred = dt_run.predict(X_test)
    ...
    save_path = '.../...'
    file_path = os.path.join(save_path, 'DT1results.csv')
    results.to_csv(file_path, index=False)

    file_path = os.path.join(save_path, 'DT1_model.pkl')
    with open(file_path, 'wb') as f:
        pickle.dump(dt_run, f)
