...
def bo_params_hgb(max_depth, min_samples_leaf, max_leaf_nodes, learning_rate):
    ...
    reg = ensemble.HistGradientBoostingRegressor(random_state=random_state, **params)
    ...
    save_path = '.../...'
    file_path = os.path.join(save_path, 'HGB2optimization_output.md')
...
def run_bayesian_optimization(X_train, y_train):
    ...
    hgb_BO.maximize(n_iter=200, init_points=20, acquisition_function=uf)
...
def cross_validate_model(X, Y, params):
    hgb_run = ensemble.HistGradientBoostingRegressor(
    ...
    for i, (tr_idx, te_idx) in enumerate(kf.split(X, Y)):
        ...
        hgb_run.set_params(random_state=random_state)
        hgb_run.fit(X_train, y_train)
        y_tr_pred = hgb_run.predict(X_train)
        y_te_pred = hgb_run.predict(X_test)
    ...
    save_path = '.../...'
    file_path = os.path.join(save_path, 'HGB2results.csv')
    results.to_csv(file_path, index=False)

    file_path = os.path.join(save_path, 'HGB2_model.pkl')
    with open(file_path, 'wb') as f:
        pickle.dump(hgb_run, f)
