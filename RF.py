...
def bo_params_rf(max_depth, min_samples_leaf, min_samples_split, max_samples, n_estimators):
    ...
    reg = ensemble.RandomForestRegressor(random_state=random_state, **params)
    ...
    save_path = '.../...'
    file_path = os.path.join(save_path, 'RF1optimization_output.md')
...
def run_bayesian_optimization(X_train, y_train):
    ...
    rf_BO.maximize(n_iter=200, init_points=20, acquisition_function=uf)
...
def cross_validate_model(X, Y, params):
    rf_run = ensemble.RandomForestRegressor(random_state=0, **params)
    ...
    for i, (tr_idx, te_idx) in enumerate(kf.split(X, Y)):
        ...
        rf_run.set_params(random_state=random_state)
